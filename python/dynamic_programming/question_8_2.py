...
def get_path(grid: List[List[bool]], row: int, col: int, path: List[Point], failed: Set[Point]):
    ...
    if (
        is_at_origin or 
        get_path(grid, row, col - 1, path, failed) or 
        get_path(grid, row - 1, col, path, failed)
    ):
        path.append(point)
        return True
